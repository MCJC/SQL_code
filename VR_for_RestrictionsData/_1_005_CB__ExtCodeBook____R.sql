/***************************************************************************************************************************************************************/
Print '--- '+CONVERT(VARCHAR(19),SYSDATETIME())+' ==>  script 1.005    ---------------------------------------------------------------------------------------- '
/***************************************************************************************************************************************************************/
/***                                                                                                                                                         ***/
...
• If one or more groups are banned, code as at least a 0.5. If there are banned groups AND a favored religion, code as 0.75. If there are banned groups AND one official religion, code as 1.
'
...
• If only approved groups are allowed to publicly preach, code as 0.50.
'
...
•	If missionaries are subject to the same visa requirements as other foreign workers, this does not count as a restriction.
...
•	If governmental officials or government-tied religious leaders must approve foreign preachers, this counts as a restriction on missionaries.
'
...
•	A threat to public order/welfare is a non-security related reason
'
...
•	This goes further than banning the group. The government should be actively attempting to eliminate the group from the country.
...
•	Record actual number. If source gives an estimate, use the lowest value of estimate.
	See General Coding Procedure in binder for details on how to count general figures like ''dozens'' or ''hundreds''.
...
•	Count each incident, not just each event. For example, if two churches were burned down in a single government action, that counts as two incidents of property damage.
...
•	Count as harassment if an incident in a prior coding period indicates ongoing trends as opposed to an isolated incident.
•	''Social groups'' does not include terrorist groups!
'
...
•	See GTD Instructions document for coding procedure. Do not record incidents of property damage, injuries, detentions, displacements, or killings from any other sources other than the GTD database or the State Department Country Terrorism Reports. From the State Department Country Terrorism Reports, only record numbers that are attributed to attacks from groups/people not on the Foreign Terrorist Organization list (i.e. lone wolf attacks like the Boston Bombers). 
•  If the country did not experience terrorist attacks in the coding year, but did in the past 5 years, check displacement numbers and record under SHI_04_d, SHI_04_d_x_1, and SHI_04_d_x_2. In these situations, code SHI_04_filter as “0.5 or higher”.
'
...
-	0
-	1 – N = 1+ cases or events were found (enter the number)
•	Government vs terrorist group = conflict (SHI_05)
•	Civilians vs terrorist group = terrorism (SHI_04)
'
,[A] =
''
...
'
Notes:
•	SHI_04_d, SHI_04_d_x_1 and SHI_04_d_x_2 answer the same question: Did religion-related terrorism contribute to individuals being displaced from their homes?
'
...
•	Consult the “SHI_04 and SHI_05” Excel file to see if country experienced terrorism/conflict in the past 5 years.
...
•	In cases where a country experienced both religion-related terrorism and conflict, divide the total displacement number in half and record for the corresponding SHI_04 and SHI_05 displacement variables.
'
...
See ''UCDP Instructions.docx'' for coding procedure!
...
•	If the country is not in a conflict in the coding year, but was in a conflict in the past 5 years, check displacement numbers and record under SHI_05_d, SHI_05_d_x_1, and SHI_05_d_x_2. In these situations, code SHI_05_filter as “1.00 - Yes”.
...
•	SHI_05_d, SHI_05_d_x_1 and SHI_05_d_x_2 answer the same question: Did religion-related war or armed conflict contribute to individuals being displaced from their homes?
